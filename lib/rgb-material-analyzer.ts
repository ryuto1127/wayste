 ...
 *  YOLO bboxes typically include 10-20% padding, so 25% ensures we sample item interior. */
const INNER_ROI_SHRINK = 0.25;
...
export function analyzeMaterial(
  ...
  if (w * h < MIN_ROI_PIXELS) {
    return { dominantHue: 0, saturation: 0, isMetallic: false, suggestedMaterial: null, bboxAspectRatio };
  }

  // Draw bbox region to a small canvas for analysis
  ...
  // Get bbox pixel data
  const roiData = ctx.getImageData(0, 0, w, h).data;
  ...
  return { dominantHue, saturation: avgSaturation, isMetallic, suggestedMaterial, bboxAspectRatio, texture };
}
...
export function refineClassName(className: string, hint: MaterialHint): string {
  ...
  // ── "cup" (not "plastic cup") + low saturation → "paper cup" ──
  // Texture confirmation: high uniformity reinforces paper
  if (lower.includes("cup") && !lower.includes("plastic") && hint.saturation < 0.2) {
  ...
  // ── white/low-saturation + tall/rectangular → "paper carton" ──
  if (hint.saturation < 0.15 && ar < 0.8) {
    if (lower.includes("box") || lower.includes("carton") || lower.includes("container")) {
      return "paper carton";
    }
  }

  // ── brown hue (15-40) + low saturation → "cardboard" ──
  if (hint.dominantHue >= 15 && hint.dominantHue <= 40 && hint.saturation < 0.3) {
