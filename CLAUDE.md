...
- Zod v4 / Jest v30 (574 tests, 30 suites) / EN+JA i18n (176 keys/locale)

## Commands
    npm run dev      # Dev server (Turbopack)
    npm run build    # Production build
    npm test         # 574 Jest tests, 30 suites
    npm run lint     # ESLint
...
- `lib/vlm-client.ts` — Tier 1.5 ローカルVLMクライアント(site config `localVlm`、ループバック限定強制)。needs_reviewトラックの切り出しをOpenAI互換ローカルエンドポイントで判定し、既存の昇格経路でカードを確定に更新
